...
$.fn.dataTableExt.oPagination.ellipses = {
    ...
    'fnInit': function (oSettings, nPager, fnCallbackDraw) {
        ...
        var oFirst = $('<li class="' + oClasses.sPageButton + ' ' + oClasses.sPageFirst + '"><a href="#">' + oLang.sFirst + '</a></li>'),
                oPrevious = $('<li class="' + oClasses.sPageButton + ' ' + oClasses.sPagePrevious + '"><a href="#">' + oLang.sPrevious + '</a>'),
                oNumbers = $('<li class="' + oClasses.sPageNumbers + '"></li>'),
                oNext = $('<li class="' + oClasses.sPageButton + ' ' + oClasses.sPageNext + '"><a href="#">' + oLang.sNext + '</a>'),
                oLast = $('<li class="' + oClasses.sPageButton + ' ' + oClasses.sPageLast + '"><a href="#">' + oLang.sLast + '</a>');
        ...
        var ul = $('<ul class="pagination" />').append(oFirst, oPrevious, oNumbers, oNext, oLast);
        // Draw
        $(nPager).append(ul);
    },
    ...
    'fnUpdate': function (oSettings, fnCallbackDraw) {
        ...
        if (oSettings._iCurrentPage === 1) {
            $('.' + oClasses.sPageFirst, tableWrapper).addClass('disabled');
        ...
        oNumbers.html('');
        $('.onumber').remove();
        $('.' + oClasses.sPageEllipsis).remove();

        for (i = oSettings._iFirstPage; i <= oSettings._iLastPage; i++) {
            oNumber = $('<li><a href="#" class="' + oClasses.sPageButton + ' ' + oClasses.sPageNumber + '">' + oSettings.fnFormatNumber(i) + '</a>');
            ...
            oNumber.addClass('onumber').insertBefore(oNumbers);
        }

        // Add ellipses
        if (1 < oSettings._iFirstPage) {
            $('<li class="' + oClasses.sPageEllipsis + '"><a>...</a></li>').insertBefore($('.onumber:first'));
        }

        if (oSettings._iLastPage < oSettings._iTotalPages) {
            $('<li class="' + oClasses.sPageEllipsis + '"><a>...</a></li>').insertAfter('.onumber:last');
        }
    },
